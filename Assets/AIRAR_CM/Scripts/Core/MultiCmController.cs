...
{
    /*
     * A sample script that can track multiple marker images.
     */
    public class MultiCmController : MonoBehaviour
    {
        [Header("▶3D Content")]
        ...
        private Dictionary<string, GameObject> trackables = new Dictionary<string, GameObject>();
        ...
        private void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs eventArgs)
        {
            ...
            foreach (ARTrackedImage added in addedImages)
            {
                ...
                if (!trackables.ContainsKey(name))
                {
                    // This code should not be deleted.
                    GameObject arContent = InstantiateWithName(added.gameObject, name);

                    trackables.Add(name, arContent);
            ...
            foreach (ARTrackedImage updated in updatedImages)
            {
                ...
                if (trackables.ContainsKey(name))
                {
                    GameObject arContent = trackables[name];

                    if (updated.trackingState == TrackingState.Tracking)
                    {
                        if (currentTrackable != null)
                        {
                            if (!currentTrackable.name.Equals(name))
                            {
                                currentTrackable.SetActive(false);
                                trackingInfo += "Lost: " + currentTrackable.name + "\n";

                                currentTrackable = arContent;
                                currentTrackable.SetActive(true);

                                trackingInfo += "Found: " + currentTrackable.name + "\n";
                            }
                        }
                        else
                        {
        ...
        // Function to instantiate and name objects at the same time
        private GameObject InstantiateWithName(GameObject parent, string name)
        {
            GameObject newObject = Instantiate(prefabs.FirstOrDefault(prefab => prefab.name.Equals(name)), parent.transform);
            newObject.name = name;
            return newObject;
        }
        ...
        public void OnClickColoring3DButton()
        {
            try
            {
                ...
                if (currentTrackable == null || !currentTrackable.activeSelf)
                {
